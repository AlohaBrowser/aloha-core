...
std::string DocumentDomain(const GURL& request_url,
  ...
  return frame_hierarchy.empty() ? std::string(request_url.host())
                                 : std::string(frame_hierarchy[0].host());
}
...
base::ListValue SubscriptionCollectionImpl::GenerateSnippets(
    const GURL& frame_url,
