...
TEST_F(AdblockSubscriptionCollectionImplTest,
  ...
  EXPECT_CALL(
      *sub1,
      HasUrlFilter(kImageAddress, std::string(frame_hierarchy[0].host()),
                   ContentType::Image, kSitekey, FilterCategory::Blocking))
  ...
  EXPECT_CALL(
      *sub1,
      HasUrlFilter(kImageAddress, std::string(frame_hierarchy[0].host()),
                   ContentType::Image, kSitekey, FilterCategory::Allowing))
      .WillOnce(Return(false));
  EXPECT_CALL(
      *sub1, HasSpecialFilter(SpecialFilterType::Document, frame_hierarchy[0],
                              std::string(frame_hierarchy[1].host()), kSitekey))
      .WillOnce(Return(false));
  EXPECT_CALL(
      *sub1, HasSpecialFilter(SpecialFilterType::Document, frame_hierarchy[1],
                              std::string(frame_hierarchy[2].host()), kSitekey))
      .WillOnce(Return(false));
  EXPECT_CALL(
      *sub1, HasSpecialFilter(SpecialFilterType::Document, frame_hierarchy[2],
                              std::string(frame_hierarchy[2].host()), kSitekey))
      .WillOnce(Return(true));
...
TEST_F(AdblockSubscriptionCollectionImplTest,
  ...
  EXPECT_CALL(*sub1,
              HasSpecialFilter(SpecialFilterType::Generichide, kParentAddress,
                               std::string(kParentAddress.host()), kSitekey))
      .WillOnce(Return(false));
  EXPECT_CALL(*sub2,
              HasSpecialFilter(SpecialFilterType::Generichide, kParentAddress,
                               std::string(kParentAddress.host()), kSitekey))
...
TEST_F(AdblockSubscriptionCollectionImplTest, GenerateSnippetsJson) {
  ...
  auto result = snippets.DebugString();
  base::TrimWhitespaceASCII(result, base::TRIM_ALL, &result);
  EXPECT_EQ("[ [ \"say\", \"Hello\" ] ]", result);
}
...
TEST_F(AdblockSubscriptionCollectionImplTest, OneHasAllowingDocumentFilter) {
  ...
  EXPECT_CALL(*sub1,
              HasSpecialFilter(SpecialFilterType::Document, kSourceUrl,
                               std::string(kParentAddress.host()), kSitekey))
      .WillOnce(Return(false));
  EXPECT_CALL(*sub1,
...
TEST_F(AdblockSubscriptionCollectionImplTest, OneHasAllowingElementHideFilter) {
  ...
  EXPECT_CALL(*sub1,
              HasSpecialFilter(SpecialFilterType::Elemhide, kSourceUrl,
                               std::string(kParentAddress.host()), kSitekey))
      .WillOnce(Return(false));
  EXPECT_CALL(*sub1,
...
TEST_F(AdblockSubscriptionCollectionImplTest, CspBlockingFilterNotFound) {
  ...
  EXPECT_CALL(*sub1,
              FindCspFilters(kImageAddress, std::string(kParentAddress.host()),
...
TEST_F(AdblockSubscriptionCollectionImplTest, CspBlockingFilterFound) {
  ...
  EXPECT_CALL(*sub1,
              HasSpecialFilter(SpecialFilterType::Document, kImageAddress,
                               std::string(kParentAddress.host()), SiteKey()))
      .WillOnce(Return(false));
  EXPECT_CALL(*sub1,
              HasSpecialFilter(SpecialFilterType::Document, kParentAddress,
                               std::string(kParentAddress.host()), SiteKey()))
...
TEST_F(AdblockSubscriptionCollectionImplTest,
  ...
  EXPECT_CALL(*sub1,
              FindCspFilters(kImageAddress, std::string(kParentAddress.host()),
                             FilterCategory::Blocking, _))
      .WillOnce(testing::WithArgs<3>([](std::set<std::string_view>& res) {
...
TEST_F(AdblockSubscriptionCollectionImplTest,
  ...
  EXPECT_CALL(*sub1,
              FindCspFilters(kImageAddress, std::string(kParentAddress.host()),
                             FilterCategory::Allowing, _))
      .WillOnce(testing::WithArgs<3>(
          [](std::set<std::string_view>& res) { res.insert("script-src *"); }));
...
TEST_F(AdblockSubscriptionCollectionImplTest, GenericBlockCspFilter) {
  ...
  EXPECT_CALL(*sub1,
      ...
      .WillOnce(testing::WithArgs<3>(
          [](std::set<std::string_view>& res) { res.insert("domain-block"); }));
...
TEST_F(AdblockSubscriptionCollectionImplTest, RewriteBlockingFilterNotFound) {
  ...
  // There are no blocking filters found.
  EXPECT_CALL(*sub1, FindRewriteFilters(kImageAddress,
                                        std::string(kParentAddress.host()),
                                        FilterCategory::Blocking))
      .WillOnce(Return(std::set<std::string_view>{}));
  EXPECT_CALL(*sub2, FindRewriteFilters(kImageAddress,
                                        std::string(kParentAddress.host()),
                                        FilterCategory::Blocking))
      .WillOnce(Return(std::set<std::string_view>{}));
