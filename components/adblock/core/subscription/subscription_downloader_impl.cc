...
std::string BuildSubscriptionQueryParams(
  ...
  return base::StrCat(
      {"lastVersion=",
       ...
       "&safe=true",
       subscription_url.has_query()
           ? ("&" + std::string(subscription_url.query()))
           : ""});
}
...
void SubscriptionDownloaderImpl::StartDownload(
  ...
  auto clean_subscription_url =
      subscription_url.ReplaceComponents(replacements);

  ongoing_downloads_[clean_subscription_url] =
      OngoingDownload{request_maker_.Run(), std::move(on_finished)};
  std::get<ResourceRequestPtr>(ongoing_downloads_[clean_subscription_url])
      ->Start(
          clean_subscription_url, AdblockResourceRequest::Method::GET,
