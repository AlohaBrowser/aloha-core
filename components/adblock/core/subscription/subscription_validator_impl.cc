...
bool IsSignatureValidInternal(
    const base::DictValue& initial_subscription_signatures,
  ...
  if (*expected_hash !=
      SubscriptionValidatorImpl::ComputeSubscriptionHash(data)) {
...
void StoreTrustedSignatureInternal(
  ...
  const auto hash = SubscriptionValidatorImpl::ComputeSubscriptionHash(data);
...
std::string SubscriptionValidatorImpl::ComputeSubscriptionHash(
    const FlatbufferData& buffer) {
