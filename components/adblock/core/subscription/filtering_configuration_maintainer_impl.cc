...
std::map<GURL, utils::DownloadStats>
FilteringConfigurationMaintainerImpl::GetDownloadStats() const {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  std::map<GURL, utils::DownloadStats> result;
  for (const auto& subscription : GetCurrentSubscriptions()) {
    const auto url = subscription->GetSourceUrl();
    result.emplace(url, utils::DownloadStats(
                            persistent_metadata_->GetDownloadSuccessCount(url),
                            persistent_metadata_->GetDownloadErrorCount(url)));
  }
  return result;
}
