...
SubscriptionServiceImpl::GetCurrentSubscriptions(
  ...
  std::ranges::transform(urls, std::back_inserter(result), [](const auto& url) {
  ...
  if (it->second) {
    ...
    for (size_t i = 0; i < result.size(); ++i) {
      auto list_it =
          std::ranges::find_if(maintainer_filter_lists, [&](const auto& entry) {
...
std::map<GURL, utils::DownloadStats> SubscriptionServiceImpl::GetDownloadStats(
    std::string_view configuration_name) const {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  auto it = std::ranges::find_if(maintainers_, [&](const auto& entry) {
    return entry.first.get()->GetName() == configuration_name;
  ...
  if (it == maintainers_.end()) {
    LOG(WARNING) << "[eyeo] Trying to get download stats for non existing "
                    "configuration: "
                 << configuration_name;
    return {};
  }
