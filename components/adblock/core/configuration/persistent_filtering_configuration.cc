...
base::DictValue ReadFromPrefs(PrefService* pref_service,
  ...
  if (this_config) {
    return base::DictValue(this_config->Clone());
  }
  return base::DictValue();
}

void StoreToPrefs(const base::DictValue& configuration,
...
void SetDefaultValuesIfNeeded(base::DictValue& configuration) {
...
bool AppendToList(base::DictValue& configuration,
                  std::string_view key,
...
bool RemoveFromList(base::DictValue& configuration,
                    std::string_view key,
                    const std::string& value) {
  DCHECK(configuration.FindList(key));  // see SetDefaultValuesIfNeeded().
  auto* list = configuration.FindList(key);
...
template <typename T>
std::vector<T> GetFromList(const base::DictValue& configuration,
                           std::string_view key) {
  DCHECK(configuration.FindList(key));  // see SetDefaultValuesIfNeeded().
