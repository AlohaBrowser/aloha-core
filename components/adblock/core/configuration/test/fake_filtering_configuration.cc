...
void FakeFilteringConfiguration::RemoveFilterList(const GURL& url) {
  filter_lists.erase(std::remove(filter_lists.begin(), filter_lists.end(), url),
                     filter_lists.end());
  if (observer) {
    observer->OnFilterListsChanged(this);
  }
...
void FakeFilteringConfiguration::RemoveAllowedDomain(
  ...
  allowed_domains.erase(
      std::remove(allowed_domains.begin(), allowed_domains.end(), domain),
      allowed_domains.end());
  if (observer) {
    observer->OnAllowedDomainsChanged(this);
  }
...
void FakeFilteringConfiguration::RemoveCustomFilter(const std::string& filter) {
  custom_filters.erase(
      std::remove(custom_filters.begin(), custom_filters.end(), filter),
      custom_filters.end());
  if (observer) {
    observer->OnCustomFiltersChanged(this);
  }
