...
void SitekeyStorageImpl::ProcessResponseHeaders(
  ...
  auto site_key = headers->GetNormalizedHeader(kSiteKeyHeaderKey);
  if (!site_key || site_key->empty()) {
    VLOG(1) << "[eyeo] No site key header";
    return;
  }

  ProcessSiteKey(request_url, SiteKey{site_key.value()}, user_agent);
}
...
void SitekeyStorageImpl::ProcessSiteKey(const GURL& request_url,
  ...
  std::string data = path_with_query;
  data += '\0';
  data += url.host();
  data += '\0';
  data += user_agent;
