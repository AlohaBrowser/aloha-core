...
// static
std::optional<DomainOption> DomainOption::FromString(
    std::string_view domains_list,
  ...
  // At least one domain should be valid.
  if (include_domains.empty() && exclude_domains.empty()) {
    return std::nullopt;
  }
