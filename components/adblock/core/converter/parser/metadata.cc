...
absl::optional<Metadata> Metadata::FromStream(std::istream& filter_stream) {
  static const base::NoDestructor<re2::RE2> comment_re(
      "^!\\s*(.*?)\\s*:\\s*(.*)");
...
bool Metadata::IsValidAdblockHeader(const std::string& adblock_header) {
  static const base::NoDestructor<re2::RE2> adblock_header_re(
      "^\\[Adblock.*\\]");
...
base::TimeDelta Metadata::ParseExpirationTime(
  ...
  static const base::NoDestructor<re2::RE2> expiration_time_re(
      "\\s*([0-9]+)\\s*(h)?.*");
