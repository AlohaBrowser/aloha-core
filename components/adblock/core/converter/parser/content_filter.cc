...
absl::optional<ContentFilter> ContentFilter::FromString(
  ...
  auto domains = DomainOption::FromString(domain_list, kDomainSeparator);
  if (!domains.has_value()) {
    VLOG(1) << "[eyeo] Invalid domain list: " << domain_list;
    return {};
  }
  ...
  return ContentFilter(filter_type, selector, modifier,
                       std::move(domains.value()));
}
