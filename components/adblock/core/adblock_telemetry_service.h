 ...
 public:
  ...
  class TopicProvider {
   public:
    ...
    // may be nullopt. Implementation is free to implement a "retry" in case of
    // response errors via GetTimeToNextRequest().
    virtual void ParseResponse(std::optional<std::string> response_content) = 0;
    ...
    virtual std::string FetchDebugInfo() const = 0;
  };
  ...
  // Collects debug information from all topic providers.
  std::vector<std::string> GetTopicProvidersDebugInfo() const;
