...
void ActivepingTelemetryTopicProvider::ParseResponse(
    std::optional<std::string> response_content) {
  if (!response_content) {
  ...
  auto parsed = base::JSONReader::ReadDict(
      *response_content, base::JSON_PARSE_CHROMIUM_EXTENSIONS);
  if (!parsed) {
    VLOG(1)
...
std::string ActivepingTelemetryTopicProvider::FetchDebugInfo() const {
  ...
  return base::WriteJsonWithOptions(debug_info,
