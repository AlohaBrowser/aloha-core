...
class MockTopicProvider
 ...
 public:
  ...
  MOCK_METHOD(void, ParseResponse, (std::optional<std::string>), (override));
  MOCK_METHOD(std::string, FetchDebugInfo, (), (const, override));
};
...
class AdblockTelemetryServiceTest : public testing::Test {
 public:
  void SetUp() override {
    static const base::NoDestructor<std::string> adblock_name(
...
TEST_F(AdblockTelemetryServiceTest, SuccessfulResponseReceived) {
  ...
  EXPECT_CALL(*mock, ParseResponse(testing::Optional(response)));
...
TEST_F(AdblockTelemetryServiceTest, CollectDebugInfoFromProviders) {
  ...
  EXPECT_CALL(*foo_mock, FetchDebugInfo())
      .WillOnce(testing::Return("foo_debug_info"));
  EXPECT_CALL(*bar_mock, FetchDebugInfo())
      .WillOnce(testing::Return("bar_debug_info"));

  auto result = telemetry_service_->GetTopicProvidersDebugInfo();
  EXPECT_THAT(result, testing::ElementsAre("foo_debug_info", "bar_debug_info"));
}
