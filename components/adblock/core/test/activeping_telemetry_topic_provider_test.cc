...
class AdblockActivepingTelemetryTopicProviderTest
 ...
 public:
  ...
  void ExpectFailureAndRetryForResponse(
      std::optional<std::string> bad_response_contents) {
...
TEST_P(AdblockActivepingTelemetryTopicProviderTest, FirstRunSucceeded) {
  ...
  provider_->ParseResponse(std::string(R"({"token":"Monday"})"));
...
TEST_P(AdblockActivepingTelemetryTopicProviderTest,
  ...
  ExpectFailureAndRetryForResponse(
      std::string(R"({"error":"1111","token":"Monday"})"));
}

TEST_P(AdblockActivepingTelemetryTopicProviderTest,
       FirstRunFailedDueToNoResponse) {
  // No server response:
  ExpectFailureAndRetryForResponse(std::nullopt);
}

TEST_P(AdblockActivepingTelemetryTopicProviderTest,
       FirstRunFailedDueToNoTokenInResponse) {
  // No "token" in server response:
  ExpectFailureAndRetryForResponse(std::string(R"({"data":"1111"})"));
}

TEST_P(AdblockActivepingTelemetryTopicProviderTest,
       FirstRunFailedDueToInvalidJsonResponse) {
  // Not a valid JSON:
  ExpectFailureAndRetryForResponse(std::string(R"(rubbish data)"));
}
...
TEST_P(AdblockActivepingTelemetryTopicProviderTest, SecondRunSucceeded) {
  ...
  provider_->ParseResponse(std::string(R"({"token":"Tuesday"})"));
  ExpectPayloadAndTimeConsistentAfterRestart();
...
TEST_P(AdblockActivepingTelemetryTopicProviderTest,
  ...
  provider_->ParseResponse(std::string(R"({"token":"Monday"})"));
  provider_->ParseResponse(std::string(R"({"token":"Tuesday"})"));
  provider_->ParseResponse(std::string(R"({"token":"Wednesday"})"));
  provider_->ParseResponse(std::string(R"({"token":"Thursday"})"));
