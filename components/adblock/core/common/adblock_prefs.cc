...
// Signature for local stores snippet library
const char kSnippetsSignature[] = "adblock.snippets.signature";

// Snippet library version
const char kSnippetsVersion[] = "adblock.snippets.version";

// Deprecated prefs registered for migration or removal.
const char kTelemetryAcceptableAdsStats[] =
    "adblock.telemetry.acceptable_ads_stats";

void MigrateDeprecatedAdblockPrefs(PrefService* pref_service) {
  // Added 03/2025, pref not used (renamed) since v131.
  pref_service->ClearPref(kTelemetryAcceptableAdsStats);
}

void RegisterDeprecatedPrefs(PrefRegistrySimple* registry) {
...
void RegisterProfilePrefs(PrefRegistrySimple* registry) {
  ...
  registry->RegisterTimePref(kAutoInstalledSubscriptionsNextUpdateTime,
                             base::Time::Now());
  registry->RegisterStringPref(kSnippetsSignature, "");
  registry->RegisterStringPref(kSnippetsVersion, "");
  RegisterTelemetryPrefs(registry);

  RegisterDeprecatedPrefs(registry);
