...
void OverrideSnippetLibraryPortForTesting(int port_for_testing) {
  g_port_for_testing = port_for_testing;
}

#include "base/no_destructor.h"

const GURL& SnippetLibraryUrl() {
  static const base::NoDestructor<GURL> kSnippetLibraryUrl(
      "https://easylist-downloads.adblockplus.org/snippets/"
#ifdef NDEBUG
      "isolated-first-all.jst"
#else
      "isolated-first-all.source.jst"
#endif
  );
  ...
  if (g_port_for_testing) {
    ...
    thread_local base::NoDestructor<GURL> overridden_url(
        kSnippetLibraryUrl->ReplaceComponents(replacements));
    *overridden_url = kSnippetLibraryUrl->ReplaceComponents(replacements);
    return *overridden_url;
  }

  return *kSnippetLibraryUrl;
}
