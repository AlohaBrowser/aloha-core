...
constexpr char kTopicSnippets[] = "snippets";
...
constexpr char kActionUpdate[] = "update";
...
std::string DecodePayload(const std::string& encoded) {
  ...
  url::DecodeUrlEscapeSequences(encoded, url::DecodeUrlMode::kUtf8OrIsomorphic,
                                &output);
  std::string decoded = base::UTF16ToUTF8(output.view());
...
std::vector<std::string> GetCommandElements(const GURL& url) {
  ...
  auto path = url.path();
  if (path.length() > 1) {
    auto path_str = path.substr(1);
    command_elements = base::SplitString(path_str, "/", base::KEEP_WHITESPACE,
...
AdblockURLLoaderFactoryForTest::AdblockURLLoaderFactoryForTest(
    ...
    : AdblockURLLoaderFactory(config,
      ...
      snippet_service_(SnippetServiceFactory::GetForBrowserContext(context)),
...
void AdblockURLLoaderFactoryForTest::CreateLoaderAndStart(
  ...
  if (!url::DomainIs(request.url.host(), kEyeoDebugDataHostName)) {
    DLOG(WARNING)
  ...
  if (!base::StartsWith(request.url.host(), kEyeoDebugDataHostName)) {
    configuration_name_ = request.url.host().substr(
        0, request.url.host().rfind(kEyeoDebugDataHostName) - 1);
...
std::string AdblockURLLoaderFactoryForTest::HandleCommand(
  ...
  if (command_elements.size() > 1) {
    ...
    } else if (topic == kTopicSnippets) {
      if (action == kActionState) {
        std::string response = kResponseOk;
        response += "\n\n";
        response += prefs_->GetString(common::prefs::kSnippetsVersion);
        response += "\n\n";
        response += "(" + snippet_service_->GetSnippetVersion() + ")";
        return response;
      } else if (action == kActionClear) {
        prefs_->ClearPref(common::prefs::kSnippetsVersion);
        return kResponseOk;
      } else if (action == kActionUpdate && command_elements.size() == 3) {
        std::string payload = DecodePayload(command_elements[2]);
        prefs_->SetString(common::prefs::kSnippetsVersion, payload);
        return kResponseOk;
      }
    }
  }
