...
void AdblockInternalsPageHandler::GetDebugInfo(GetDebugInfoCallback callback) {
  ...
  if (!service || !stats) {
    std::move(callback).Run("No subscription service or session stats found");
    return;
  }
  ...
  auto* snippets =
      adblock::SnippetServiceFactory::GetForBrowserContext(context_);
  content += DebugLine("", "Snippets library information", 0);
  content += DebugLine("ETag version",
                       prefs->GetString(common::prefs::kSnippetsVersion), 1);
  content += DebugLine("Release version", snippets->GetSnippetVersion(), 1);
