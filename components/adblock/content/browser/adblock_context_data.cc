...
void AdblockContextData::StartProxying(
  ...
  std::string user_agent_str = web_contents->GetUserAgentOverride().ua_string_override;  // ALOHA https://app.clickup.com/t/862khu08c
  ...
  if (use_test_loader) {
    auto proxy = std::make_unique<adblock::AdblockURLLoaderFactoryForTest>(
        ...
        content::GlobalRenderFrameHostId(render_process_id,
                                         frame->GetRoutingID()),
        std::move(receiver), std::move(target_factory), user_agent_str,
