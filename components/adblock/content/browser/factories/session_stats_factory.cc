...
SessionStatsFactory::BuildServiceInstanceForBrowserContext(
  ...
  auto session_stats_service = std::make_unique<SessionStatsImpl>(
      ResourceClassificationRunnerFactory::GetForBrowserContext(context));

  KeyedServicePtrHolder<SessionStats>::Set(
      context, kSessionStatsServiceUserDataKey, session_stats_service.get());
