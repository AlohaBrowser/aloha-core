  ...
  if (configuration->GetName() == kAdblockFilteringConfigurationName) {
    recommended_installer =
        std::make_unique<RecommendedSubscriptionInstallerImpl>(
            ...
            base::BindRepeating(
                &SubscriptionServiceFactory::MakeSubscriptionRequest, context));
  }
...
SubscriptionServiceFactory::BuildServiceInstanceForBrowserContext(
  ...
  KeyedServicePtrHolder<SubscriptionService>::Set(
      context, kSubscriptionServiceUserDataKey, subscription_service.get());
...
std::unique_ptr<AdblockResourceRequest>
SubscriptionServiceFactory::MakeSubscriptionRequest(
    content::BrowserContext* context) {
