...
void TestResourceClassificationRunnerObserver::OnRequestMatched(
  ...
  if (match_result == FilterMatchResult::kAllowRule) {
    allowed_ads_notifications_.push_back(url);
  } else {
...
void TestResourceClassificationRunnerObserver::OnPageAllowed(
    ...
    const GURL& subscription,
    const std::string& configuration_name) {
  allowed_pages_notifications_.push_back(url);
}
...
void TestResourceClassificationRunnerObserver::OnPopupMatched(
    ...
    const std::string& configuration_name) {
  if (match_result == FilterMatchResult::kAllowRule) {
    allowed_popups_notifications_.push_back(url);
  } else {
    blocked_popups_notifications_.push_back(url);
  }
}

void TestResourceClassificationRunnerObserver::OnPageElementMatched(
    const std::string& selector,
    ElementHideAction action,
    content::RenderFrameHost* render_frame_host) {
  switch (action) {
    case ElementHideAction::kHide:
      hidden_notifications_.push_back(selector);
      break;
    case ElementHideAction::kRemove:
      removed_notifications_.push_back(selector);
      break;
    case ElementHideAction::kInlineCss:
      inline_css_notifications_.push_back(selector);
      break;
    default:
      NOTREACHED();
  }
...
void SubscriptionInstalledWaiter::OnSubscriptionInstalled(
  ...
  awaited_subscriptions_.erase(
      std::remove(awaited_subscriptions_.begin(), awaited_subscriptions_.end(),
                  subscription_url),
...
const std::string& AdblockBrowserTestBase::GetTelemetryDomain() {
  static const base::NoDestructor<std::string> domain(
