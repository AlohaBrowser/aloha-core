 ...
 public:
  AdblockFilteringConfigurationBrowserTest()
      : prerender_test_helper_(
            base::BindRepeating(&AdblockBrowserTestBase::web_contents,
  ...
  bool IsResourceLoaded() {
    const std::string javascript =
        "(document.getElementById('subresource') &&"
        "document.getElementById('subresource').naturalHeight !== 0)";
    return WaitAndVerifyCondition(javascript.c_str());
  }
...
// See DPD-2890
IN_PROC_BROWSER_TEST_P(AdblockFilteringConfigurationBrowserTest,
                       ResourceBlockedInPrerenderedFrame) {
  // We just need a starting page, let's use tab-restore.html as a starting
  // point and prerender innermost_frame.html.
  GURL url_a(embedded_test_server()->GetURL("test.org", "/tab-restore.html"));
  GURL url_b(
      embedded_test_server()->GetURL("test.org", "/innermost_frame.html"));
  GURL subresource_b(
      embedded_test_server()->GetURL("test.org", "/resource.png"));
  auto configuration = MakeConfiguration("config");
  ...
  ASSERT_EQ(observer_.blocked_ads_notifications_.size(), 0u);
  // This call creates and injects <script type="speculationrules">
  auto frame_id = prerender_test_helper().AddPrerender(url_b);
  prerender_test_helper().WaitForPrerenderLoadCompletion(frame_id);
  ASSERT_EQ(observer_.blocked_ads_notifications_.size(), 1u);
  EXPECT_TRUE(observer_.blocked_ads_notifications_.front() == subresource_b)
      << "Request not blocked!";
  ExpectResourceBlocked();
  auto* rfh = prerender_test_helper().GetPrerenderedMainFrameHost(
...
IN_PROC_BROWSER_TEST_P(AdblockFilteringConfigurationBrowserTest,
  ...
  auto configuration = MakeConfiguration("config");
  configuration->AddCustomFilter("*resource.png");
  configuration->AddAllowedDomain(std::string(GetPageUrl().host()));
...
IN_PROC_BROWSER_TEST_P(AdblockFilteringConfigurationBrowserTest,
  ...
  auto configuration = std::ranges::find(configurations, "persistent",
...
IN_PROC_BROWSER_TEST_P(AdblockFilteringConfigurationBrowserTest,
  ...
  configurations =
      SubscriptionServiceFactory::GetForBrowserContext(browser_context())
          ->GetInstalledFilteringConfigurations();
  configuration = std::ranges::find(configurations, "persistent",
                                    &FilteringConfiguration::GetName);
  ASSERT_TRUE(configuration == configurations.end());
}
...
IN_PROC_BROWSER_TEST_P(AdblockFilteringConfigurationBrowserTest,
  ...
  auto configurations =
      SubscriptionServiceFactory::GetForBrowserContext(browser_context())
          ->GetInstalledFilteringConfigurations();
  auto configuration =
      std::ranges::find(configurations, kAdblockFilteringConfigurationName,
                        &FilteringConfiguration::GetName);
  ASSERT_TRUE(configuration != configurations.end());
