...
class AdblockSubscriptionServiceBrowserTest
 ...
 public:
  ...
  bool RequestHeadersContainAcceptEncodingBrotli(
    ...
    return std::ranges::find(split_encodings, "br") != split_encodings.end();
  }
...
class AdblockSubscriptionServiceDownloadStatsBrowserTest
    : public AdblockBrowserTestBase {
 public:
  AdblockSubscriptionServiceDownloadStatsBrowserTest()
    ...
    https_server_.RegisterRequestHandler(base::BindRepeating(
        &AdblockSubscriptionServiceDownloadStatsBrowserTest::RequestHandler,
  ...
  void SetUpOnMainThread() override {
    ...
    for (const auto& filter_list :
         GetAdblockFilteringConfiguration()->GetFilterLists()) {
      GetAdblockFilteringConfiguration()->RemoveFilterList(filter_list);
    }
  }
  ...
  std::unique_ptr<net::test_server::HttpResponse> RequestHandler(
    ...
    auto http_response =
        std::make_unique<net::test_server::BasicHttpResponse>();
    http_response->set_code(http_response_code_);
    http_response->set_content("[Adblock Plus 2.0]\n");
    http_response->set_content_type("text/plain");
    return http_response;
  }

  void AddFilterList(const GURL& url) {
    GetAdblockFilteringConfiguration()->AddFilterList(url);
    auto waiter = GetSubscriptionInstalledWaiter();
    waiter.WaitUntilSubscriptionsInstalled({url});
  }
 ...
 protected:
  ...
  net::HttpStatusCode http_response_code_ = net::HTTP_OK;
  net::EmbeddedTestServer https_server_;
};

IN_PROC_BROWSER_TEST_F(AdblockSubscriptionServiceDownloadStatsBrowserTest,
                       SuccessDownload) {
  auto* subscription_service =
      SubscriptionServiceFactory::GetForBrowserContext(browser_context());

  auto download_stats = subscription_service->GetDownloadStats(
      kAdblockFilteringConfigurationName);
  EXPECT_TRUE(download_stats.empty());

  AddFilterList(DefaultSubscriptionUrl());
  download_stats = subscription_service->GetDownloadStats(
      kAdblockFilteringConfigurationName);
  for (const auto& [url, stats] : download_stats) {
    LOG(ERROR) << "URL: " << url.spec() << " : " << stats.success_count
               << stats.error_count;
  }
  EXPECT_EQ(download_stats.size(), 1u);
  EXPECT_THAT(download_stats,
              testing::Contains(std::make_pair(
                  DefaultSubscriptionUrl(),
                  utils::DownloadStats{.success_count = 1, .error_count = 0})));
}
