...
class AdblockPageViewStatsBrowserTest : public AdblockBrowserTestBase {
 public:
  ...
  std::unique_ptr<net::test_server::HttpResponse> RequestHandler(
    ...
    if (base::StartsWith(request.relative_url, "/redirect_to")) {
      GURL redirect_target("https://example.com/" +
                           std::string(request.GetURL().query()));
    ...
    if (base::StartsWith(request.relative_url,
      ...
      telemetry_response_ = base::JSONReader::Read(
 ...
 protected:
  ...
  base::DictValue* GetPayload() {
    ...
    base::DictValue* parsed_dict = telemetry_response_->GetIfDict();
    EXPECT_TRUE(parsed_dict);
    base::DictValue* payload = parsed_dict->FindDict("payload");
    EXPECT_TRUE(payload);
    return payload;
  }
