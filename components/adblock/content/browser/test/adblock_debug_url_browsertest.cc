...
IN_PROC_BROWSER_TEST_P(AdblockDebugUrlTest, TestSnippetsCommands) {
  GURL show_url(GetUrlForAdblockConfiguration() + "/snippets/state");
  GURL clear_url(GetUrlForAdblockConfiguration() + "/snippets/clear");
  GURL update_url(GetUrlForAdblockConfiguration() +
                  "/snippets/update/12345678");

  ASSERT_TRUE(content::NavigateToURL(shell(), show_url));
  ASSERT_TRUE(base::StartsWith(
      ExecuteScriptAndExtractString(kReadPageBodyScript), "OK\n\n"));

  ASSERT_TRUE(content::NavigateToURL(shell(), update_url));
  ASSERT_EQ("OK", ExecuteScriptAndExtractString(kReadPageBodyScript));
  ...
  ASSERT_TRUE(content::NavigateToURL(shell(), clear_url));
  ASSERT_EQ("OK", ExecuteScriptAndExtractString(kReadPageBodyScript));
