...
#include "components/adblock/core/snippet/test/mock_snippet_service.h"
...
class AdblockElementHiderImplTest : public content::RenderViewHostTestHarness {
 protected:
  void SetUp() override {
    ...
    EXPECT_CALL(mock_delegate_, LoadDataResourceString(IDR_ADBLOCK_LOG_EH_JS))
        .WillRepeatedly(testing::Return("{{elemHidingEmulatedPatternsDef}}"));
  }
  ...
  void ExpectSnippetsGenerated() {
    EXPECT_CALL(snippet_service_, GenerateSnippetScript(testing::_, testing::_))
        .WillOnce([&](const GURL&, base::ListValue input) {
  ...
  MockSnippetService snippet_service_;
...
TEST_F(AdblockElementHiderImplTest, BatchesSelectors) {
  ...
  ElementHiderImpl element_hide(&sub_service_, &snippet_service_);
          ...
          [&](const ElementHider::ElemhideInjectionData& data) {
            ...
            for (auto& line : lines) {
              ...
              EXPECT_TRUE(base::EndsWith(
                  line,
                  "{display: none !important; content: \"eyeo\" !important;}"));
            }
          }));
}
...
TEST_F(AdblockElementHiderImplTest, AppliesElementHidingOnSiteWithWeirdUrl) {
  ...
  element_hide.ApplyElementHidingEmulationOnPage(
      ...
      base::BindLambdaForTesting([&](const ElementHider::ElemhideInjectionData&
        ...
        EXPECT_EQ(
            data.stylesheet,
            "a, b {display: none !important; content: \"eyeo\" !important;}\n");
      }));
}
...
TEST_F(AdblockElementHiderImplTest, GeneratesSnippetsWhenEhAllowListed) {
  EXPECT_CALL(sub_service_, GetCurrentSnapshot()).WillOnce([this]() {
    ...
    base::ListValue snippets;
    snippets.Append(base::ListValue());
...
TEST_F(AdblockElementHiderImplTest, UsesTwoConfigs) {
  ...
  base::ListValue snippets_config_1;
  ...
  base::ListValue snippets_config_2;
  ...
  element_hide.ApplyElementHidingEmulationOnPage(
      ...
      base::BindLambdaForTesting(
          [&](const ElementHider::ElemhideInjectionData& data) {
            EXPECT_EQ(data.stylesheet,
                      "a1, b1, a2, b2 {display: none !important; content: "
                      "\"eyeo\" !important;}\n");
            EXPECT_EQ(data.elemhide_js,
                      "{selector:\"c1\"}, \n{selector:\"d1\"}, "
                      "\n{selector:\"c2\"}, \n{selector:\"d2\"}, \n");
...
TEST_F(AdblockElementHiderImplTest, UsesSecondConfigWhenFirstAllowlisted) {
  ...
  element_hide.ApplyElementHidingEmulationOnPage(
      ...
      base::BindLambdaForTesting([&](const ElementHider::ElemhideInjectionData&
                                         data) {
        EXPECT_EQ(data.stylesheet,
                  "a2, b2 {display: none !important; content: \"eyeo\" "
                  "!important;}\n");
        EXPECT_EQ(data.elemhide_js,
                  "{selector:\"c2\"}, \n{selector:\"d2\"}, \n");
        EXPECT_EQ(
