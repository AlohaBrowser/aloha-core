...
void ResourceClassificationRunnerImpl::NotifyPageElementMatched(
  ...
  switch (action) {
    ...
    case ElementHideAction::kInlineCss:
      action_str = "inline CSS targetted";
      break;
    default:
      action_str = "unknown";
      NOTREACHED();
  }
  VLOG(1) << "[eyeo] NotifyPageElementMatched() called for action "
          << action_str << " selector " << selector << " at "
          << render_frame_host->GetLastCommittedURL();
  for (auto& observer : observers_) {
    observer.OnPageElementMatched(selector, action, render_frame_host);
  }
}
