...
public class AdblockController {
    ...
    private final FilteringConfiguration mFilteringConfiguration;
    ...
    public List<Subscription> getRecommendedSubscriptions() {
        Object[] raw = AdblockControllerJni.get().getRecommendedSubscriptions();
        List<Subscription> result = new ArrayList<>(raw.length);
        for (Object item : raw) result.add((Subscription) item);
        return result;
    }
    ...
    public List<Subscription> getInstalledSubscriptions() {
        Object[] raw = AdblockControllerJni.get().getInstalledSubscriptions(mBrowserContextHandle);
        List<Subscription> result = new ArrayList<>(raw.length);
        for (Object item : raw) result.add((Subscription) item);
        return result;
    }
