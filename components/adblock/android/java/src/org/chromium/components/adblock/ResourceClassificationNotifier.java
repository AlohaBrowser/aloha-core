...
public class ResourceClassificationNotifier {
    ...
    public interface ResourceFilteringObserver {
        ...
        /**
         * "Page element matched" event for a page element which was targeted by EH or EHE filter.
         *
         * <p>It should not block the UI thread for too long.
         *
         * @param info contains auxiliary information about the resource.
         */
        @UiThread
        default void onPageElementMatched(ResourceFilteringCounters.PageElementInfo info) {}
    }
    ...
    @CalledByNative
    private void pageElementMatchedCallback(
            final String selector,
            final int action,
            final int renderProcessId,
            final int renderFrameId) {
        ThreadUtils.assertOnUiThread();
        final ResourceFilteringCounters.PageElementInfo pageElementInfo =
                new ResourceFilteringCounters.PageElementInfo(
                        selector,
                        action,
                        new GlobalRenderFrameHostId(renderProcessId, renderFrameId));
        Log.d(
                TAG,
                "eyeo: pageElementMatchedCallback() notifies "
                        + mResourceFilteringObservers.size()
                        + " listeners about "
                        + pageElementInfo.toString());
        for (final ResourceFilteringObserver observer : mResourceFilteringObservers) {
            observer.onPageElementMatched(pageElementInfo);
        }
    }
