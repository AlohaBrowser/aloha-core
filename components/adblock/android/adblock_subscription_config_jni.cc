...
ScopedJavaLocalRef<jobject> ToJava(JNIEnv* env,
  ...
  ScopedJavaLocalRef<jobject> url_param = ScopedJavaLocalRef<jobject>::Adopt(
...
JNI_AdblockSubscriptionConfig_GetRecommendedSubscriptions(JNIEnv* env) {
  ...
  ScopedJavaLocalRef<jclass> subscription_class = GetClass(
      env,
      "org/chromium/components/adblock/AdblockSubscriptionConfig$Subscription");
  return ToJavaArrayOfObjects(env, subscription_class.obj(),
                              CSubscriptionsToJObjects(env, list));
}
DEFINE_JNI(AdblockSubscriptionConfig)