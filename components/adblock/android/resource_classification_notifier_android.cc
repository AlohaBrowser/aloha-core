...
void ResourceClassificationNotifierAndroid::OnPopupMatched(
  ...
  if (!java_controller.is_null()) {
    ...
    Java_ResourceClassificationNotifier_popupMatchedCallback(
        ...
        j_configuration, rfh_id.child_id.GetUnsafeValue(),
        rfh_id.frame_routing_id);
  }
}

void ResourceClassificationNotifierAndroid::OnPageElementMatched(
    const std::string& selector,
    ElementHideAction action,
    content::RenderFrameHost* render_frame_host) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK(render_frame_host);
  DVLOG(3) << "[eyeo] Page element matched " << selector;
  JNIEnv* env = AttachCurrentThread();
  auto java_controller = java_weak_controller_.get(env);
  if (!java_controller.is_null()) {
    ScopedJavaLocalRef<jstring> j_selector =
        ConvertUTF8ToJavaString(env, selector);
    const content::GlobalRenderFrameHostId rfh_id =
        render_frame_host->GetGlobalId();
    Java_ResourceClassificationNotifier_pageElementMatchedCallback(
        env, java_controller, j_selector, static_cast<int>(action),
        rfh_id.child_id.GetUnsafeValue(), rfh_id.frame_routing_id);
  }
}
}  // namespace adblock
