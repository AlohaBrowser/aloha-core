...
const base::flat_map<std::string_view, ContentType>& GetMapping() {
  static const base::NoDestructor<base::flat_map<std::string_view, ContentType>>
      kMapping({{"font", ContentType::Font},
...
absl::optional<ContentType> StringToContentType(
  ...
  const auto& kMapping = GetMapping();
