...
CookieManager* CookieManager::GetInstance(int inst_num) {
  static std::array<CookieManager*,aloha::kCookieManagersCount> instances = {nullptr};
  if (instances.at(0) == nullptr) {
    for(size_t i = 0; i < instances.size(); i++) {
      // ALOHA: pass nullptr, NOT AwBrowserContext::GetDefault(). These are the
      instances.at(i) = new CookieManager(nullptr, i);
    }
  }
  return instances.at(inst_num);
}
...
aloha::CookieType CookieManager::GetActiveCookieType() {
  return active_cookie_manager_num >= 0
             ? static_cast<aloha::CookieType>(active_cookie_manager_num)
             : aloha::kDefaultCookieManager;
}
...
void CookieManager::AddCookieFromMigration(
    ...
    jint priority,
    jint source_scheme,
    const JavaRef<jobject>& java_error_callback) {

  DCHECK(java_error_callback);
  ...
  std::unique_ptr<net::CanonicalCookie> cookie =
      net::CanonicalCookie::FromStorage(
          ...
          static_cast<net::CookieSourceScheme>(source_scheme),
          url::PORT_UNSPECIFIED, net::CookieSourceType::kOther, net::CanonicalCookieFromStorageCallSite::kCookieManager); // ALOHA https://app.clickup.com/t/86eqcg80j
...
void CookieManager::AddCookieFromMigrationImpl(
  ...
  if (auto* mojo_manager = GetMojoCookieManager()) {
    ...
    GetCookieStore()->SetCanonicalCookieAsync(std::move(cookie), source_url,
      net::CookieOptions::MakeAllInclusive(), std::move(callback), /*cookie_access_result=*/std::nullopt);
  }
}
...
CookieManager::CookieManager(AwBrowserContext* const parent_context, int inst_num)
    : parent_context_(parent_context),
      allow_file_scheme_cookies_(kDefaultFileSchemeAllowed),
      cookie_store_created_(false),
      workaround_http_secure_cookies_(
  ...
  // ALOHA - Cookies https://app.clickup.com/t/2dmr616
  cookie_store_path_ = GetContextPath().Append(FILE_PATH_LITERAL("bromium/Cookies")).AddExtension(std::to_string(inst_num));
  //cookie_store_path_ = GetPathInAppDirectory()
...
static jlong JNI_AwCookieManager_GetPublicCookieManager(JNIEnv* env) {
  return reinterpret_cast<intptr_t>(CookieManager::GetInstance(aloha::kPublicCookieManager));
}
...
static jlong JNI_AwCookieManager_GetActive(JNIEnv* env) {
  if (active_cookie_manager_num == aloha::kPublicCookieManager) {
