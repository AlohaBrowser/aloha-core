...
bool AdblockAwContentBrowserClient::CanCreateWindow(
  ...
  if (IsFilteringNeeded(opener, opener->GetBrowserContext())
      && web_contents && web_contents->IsAdblockEnabled()) { // ALOHA https://app.clickup.com/t/86epcdndk
...
bool AdblockAwContentBrowserClient::IsFilteringNeeded(
  ...
  // ALOHA fix crash when AwSettings is null, we already had this crash for enabled/disabled adblock for the one AwContents instance.
  // ALOHA https://app.clickup.com/t/86eub1cwp
  // if (frame) {
  //   auto* wc = content::WebContents::FromRenderFrameHost(frame);
  //   auto* aw = AwContents::FromWebContents(wc);
