...
class DevToolsEyeoHandlerBrowserTest
 ...
 public:
  ...
  const base::DictValue* TriggerCommand(
      std::string cmd,
  ...
  std::vector<std::string> ExtractResponseData(const base::DictValue* result,
                                               const std::string& key) {
    auto* data = result->FindList(key);
    EXPECT_TRUE(data);
    std::vector<std::string> actual_values;
    if (data) {
      for (base::ListValue::const_iterator it = data->cbegin();
           it != data->cend(); ++it) {
  ...
  void WaitForEventNotification(const std::string& event_name,
    ...
    base::DictValue notification = WaitForNotification(event_name, true);
