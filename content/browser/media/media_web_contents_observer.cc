...
// ALOHA https://app.clickup.com/t/86ewcm2rh
#include "aloha/src/native/player/player_base.h"
#include "aloha/src/native/player/player_youtube.h"
    ...
    if (attr_it != media_player_attributes_.end()) {
      ...
      if (client_bridge != nullptr) {
        client_bridge->OnMediaDestroy(
            ...
            player_id.frame_routing_id.child_id.GetUnsafeValue(),
            player_id.frame_routing_id.frame_routing_id, player_id.player_id,
...
void MediaWebContentsObserver::MediaPlayerObserverHostImpl::OnMediaPlaying(
  ...
  if (client_bridge != nullptr) {
    ...
    auto* web_contents = media_web_contents_observer_->web_contents_impl();
    // ALOHA https://app.clickup.com/t/86ewcm2rh fix background video play for YouTube
    if (aloha::player::IsYouTube(media_url.possibly_invalid_spec())) {
      web_contents->GetPrimaryMainFrame()->ExecuteJavaScript(
          base::UTF8ToUTF16(aloha::player::PlayerYouTube::GetJSFixForBackgroundVideoPlay(should_play_background_video)), base::NullCallback());
    }
    ...
    client_bridge->OnMediaPlay(
        ...
        media_player_id_.frame_routing_id.child_id.GetUnsafeValue(), media_player_id_.frame_routing_id.frame_routing_id, media_player_id_.player_id,
...
void MediaWebContentsObserver::OnMediaEffectivelyFullscreenChanged(
  ...
  if (client_bridge != nullptr) {
    client_bridge->OnVideoFullscreenChanged(
        ...
        player_id.frame_routing_id.child_id.GetUnsafeValue(), player_id.frame_routing_id.frame_routing_id, player_id.player_id,
