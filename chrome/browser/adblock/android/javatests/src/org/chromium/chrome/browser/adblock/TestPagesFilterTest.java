...
@CommandLineFlags.Add({
    ...
    AdblockSwitches.DISABLE_EYEO_SNIPPETS_LIBRARY_UPDATE
})
public class TestPagesFilterTest extends TestPagesFilterTestBase {
    ...
    public void testVerifyPopupFilters() throws Exception {
        ...
        // Wait for all tabs to actually be removed from the tab model, since tab closure callbacks
        // is fired before accual tab removal.
        final long startTime = System.currentTimeMillis();
        final long timeoutMs = TestPagesHelper.TEST_TIMEOUT_SEC * 1000;
        while (mHelper.getTabCount() != 1) {
            if (System.currentTimeMillis() - startTime > timeoutMs) {
                int tabCount = mHelper.getTabCount();
                StringBuilder errorMsg = new StringBuilder();
                errorMsg.append("Expected 1 tab but found ")
                        .append(tabCount)
                        .append(". Open tab URLs:\n");
                for (int i = 0; i < tabCount; i++) {
                    String url =
                            mActivityTestRule
                                    ...
                                    .getUrl()
                                    .getSpec();
                    errorMsg.append("  [").append(i).append("]: ").append(url).append("\n");
                }
