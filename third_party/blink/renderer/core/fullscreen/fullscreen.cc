...
std::optional<aloha::FullscreenVideoElement> Fullscreen::SelectFullscreenVideoElementInfo(
  ...
  aloha::FullscreenStateHandler::GetInstance()->Reset(); // ALOHA https://app.clickup.com/t/86eppm2e8
  ...
  aloha::ForeachByVideoElements(
    ...
    [&] (blink::HTMLVideoElement& video_elem) {
      ...
      if (const auto* media_player = video_elem.GetWebMediaPlayer()) {
        ...
        if (!url.IsEmpty()) {
          aloha::FullscreenStateHandler::GetInstance()->PrepareForFullscreen(&video_elem, url.GetString().GetString().Utf8()); // ALOHA https://app.clickup.com/t/86eppm2e8
  ...
  if (elements.size() == 1) {
    ...
    if(aloha::FullscreenStateHandler::GetInstance()->GetType() == aloha::player::PlayerType::kJwPlayer)
    {
      ...
      auto* observer = MutationObserver::Create(
          ...
          base::BindRepeating([](blink::Element& video_element){ // ALOHA https://app.clickup.com/t/86eppm2e8 callback, when style attribute is changed
            ...
            if(aloha::FullscreenStateHandler::GetInstance()->GetType() == aloha::player::PlayerType::kJwPlayer) { // ALOHA https://app.clickup.com/t/86eppm2e8
...
ScriptPromise<IDLUndefined> Fullscreen::ExitFullscreen(Document& doc,
  ...
  aloha::FullscreenStateHandler::GetInstance()->ExitFullscreen(FullscreenElementFrom(doc));
  if(aloha::FullscreenStateHandler::GetInstance()->IgnoreExitFullscreen(script_state, exception_state)) // ALOHA https://app.clickup.com/t/86eppm2e8
