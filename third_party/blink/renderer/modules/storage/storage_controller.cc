...
void StorageController::ClearStoragesForSite(const String& site) {
  for (const auto& session_namespace : namespaces_->Values()) {
    session_namespace->ClearStoragesForSite(site);
  }

  if (public_local_storage_namespace_)
      public_local_storage_namespace_->ClearStoragesForSite(site);
  if (private_local_storage_namespace_)
      private_local_storage_namespace_->ClearStoragesForSite(site);
}
